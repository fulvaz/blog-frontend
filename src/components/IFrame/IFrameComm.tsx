...
interface IframeCommType {
  postMessageData?: string;
  handleReceiveMessage?: (msg: string) => void;
  handleReady?: () => {};
  attributes?: any;
...
export class IframeComm extends React.Component<IframeCommType> {
  static defaultProps = {
  ...
  sendMessage(postMessageData) {
    ...
    if (targetOrigin === '*') {
        console.error('IFrame: you should not pass "*" as targetOrigin to postMessage ');
    }
