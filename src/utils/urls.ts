      ...
      if (typeof obj[key] === `object`) {
        if (!Array.isArray(obj[key])) {
          ...
          params[`${namespace}-${key}`] =
            obj[key].length > 0 ? obj[key].join(`,`) : null;
        }
      } else {
...
function CreateObject(obj: {}, namespace?: string) {
  ...
  Object.keys(obj).forEach((key: string) => {
    if ((namespace && key.indexOf(namespace) === 0) || !namespace) {
      ...
      arr.splice(0, 1);
      if (params[arr[0]]) {
        params[arr[0]] = AssignObject(
          params[arr[0]],
          ArrToObject(arr, obj[key])[arr[0]]
        );
      } else {
...
export function putStateInUrl(
  namespace: string,
  config: PutStateInUrlConfig,
  iSDav?: boolean
): any {
  return function(target): any {
    ...
    target.prototype.componentDidMount = function(...args) {
      ...
      this[DEFAULT_STATE_KEY] = JSON.parse(
        JSON.stringify(iSDav ? this.props : this.state)
      );
      /*初始化部分，将url映射到state */
      const {
      ...
      Object.keys(config).forEach((method: string) => {
        ...
        const start = AssignObject(
          CreateObject(params, namespace),
          ...
          Object.keys(prev_params).forEach(prev => {
            Object.keys(curr_params).forEach(curr => {
              if (
                ...
                if (
                  prev_params[curr] !== params[curr] ||
                  prev_params[prev] !== params[prev]
                ) {
                  changeParams.push(true);
                }
              }
  ...
  return function(
    ...
    descriptor.value = function(...args) {
      ...
      config.filters.forEach(filter => {
        const params = FindObject(
          config.namespace,
          config.isDav ? this.props : this.state,
          filter
        );
